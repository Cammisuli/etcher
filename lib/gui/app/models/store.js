...
const targetLedsMap = require('../modules/target-leds-map.ts')
...
const storeReducer = (state = DEFAULT_STATE, action) => {
  switch (action.type) {
    case ACTIONS.SET_AVAILABLE_DRIVES: {
      ...
      // Check drives and turn on leds connected
      console.log('targetLedsMap', targetLedsMap)
      console.log('nonStaleNewState.availableDrives', JSON.stringify(nonStaleNewState.toJS().availableDrives))
